...
func Create{Table}(e *{Table}) (*{Table}, error) {
	return DBCreate{Table}(DB({Table}Table), e)
}

func DBCreate{Table}(db *gorm.DB, e *{Table}) (*{Table}, error) {
...
func Get{Table}(id int64) (*{Table}, error) {
	return DBGet{Table}(DB({Table}Table), id)
}

func DBGet{Table}(db *gorm.DB, id int64) (*{Table}, error) {
	e := &{Table}{}
	if err := db.Where("id = ?", id).First(e).Error; err != nil {
		logs.Error("get {table} fails: %s", id, err.Error())
		return nil, err
	}
	return e, nil
}

func Get{Table}s(m map[string]interface{}) ([]*{Table}, error) {
	return DBGet{Table}s(DB({Table}Table), m)
}

func DBGet{Table}s(db *gorm.DB, m map[string]interface{}) ([]*{Table}, error) {
	result := []*{Table}{}
...
func Get{Table}sByIds(ids []int64) ([]*{Table}, error) {
	return DBGet{Table}sByIds(DB({Table}Table), ids)
}

func DBGet{Table}sByIds(db *gorm.DB, ids []int64) ([]*{Table}, error) {
	result := []*{Table}{}
...
func Delete{Table}(id int64) error {
	return DBDelete{Table}(DB({Table}Table), id)
}

func DBDelete{Table}(db *gorm.DB, id int64) error {
...
func Get{Table}sByTime(start time.Time, end time.Time, m map[string]interface{}, rawq string, limit int, getCount bool) ([]*{Table}, int, error) {
	return DBGet{Table}sByTime(DB({Table}Table), start, end, m, rawq, limit, getCount)
}

func DBGet{Table}sByTime(db *gorm.DB, start time.Time, end time.Time, m map[string]interface{}, rawq string, limit int, getCount bool) ([]*{Table}, int, error) {
	totalCount := 0
	result := make([]*{Table}, 0)
	db = db.Model({Table}{})
	if m != nil {
...
func Get{Table}sById(id int64, m map[string]interface{}, rawq string, dir int, order int, ordcond string, limit int, getCount bool) ([]*{Table}, int, error) {
	return DBGet{Table}sById(DB({Table}Table), id, m, rawq, dir, order, ordcond, limit, getCount)
}

func DBGet{Table}sById(db *gorm.DB, id int64, m map[string]interface{}, rawq string, dir int, order int, ordcond string, limit int, getCount bool) ([]*{Table}, int, error) {
	totalCount := 0
	result := make([]*{Table}, 0)
	db = db.Model({Table}{})
	if m != nil {
...
func Get{Table}sByOffset(offset int, m map[string]interface{}, rawq string, order int, ordcond string, limit int, getCount bool) ([]*{Table}, int, error) {
	return DBGet{Table}sByOffset(DB({Table}Table), offset, m, rawq, order, ordcond, limit, getCount)
}

func DBGet{Table}sByOffset(db *gorm.DB, offset int, m map[string]interface{}, rawq string, order int, ordcond string, limit int, getCount bool) ([]*{Table}, int, error) {
	totalCount := 0
	result := make([]*{Table}, 0)
	db = db.Model({Table}{})
	if m != nil {
