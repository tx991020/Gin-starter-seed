...
func loadConfig() (*config.Config, error) {
	...
	if configPath != "" {
		conf, _ := config.NewFromYaml(configPath, true)
		fmt.Println(222, conf)
...
func send() error {
	...
	var task1 []*tasks.Signature
	for i := 0; i < 100; i++ {
		task1 = append(task1, &tasks.Signature{
			Name: "crawler",
			Args: []tasks.Arg{
				{
					Type:  "string",
					Value: fmt.Sprintf("https://hk.saowen.com/page/%d", i),
				},
			},
		})

	}
	group, err := tasks.NewGroup(task1...)
	...
	//asyncResult, err := server.SendTaskWithContext(ctx, tasks1)
	//if err != nil {
